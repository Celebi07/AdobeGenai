...
def save_results_to_csv(rectangles, lines, circles, output_csv_path):
    with open(output_csv_path, 'w') as f:
        if rectangles is not None:
            for rect in rectangles:
                f.write(f"Rectangle,{','.join(map(str, rect.flatten()))}\n")
        if lines is not None:
            for line in lines:
                f.write(f"Line,{','.join(map(str, line.flatten()))}\n")
        if circles is not None:
            for circle in circles:
                f.write(f"Circle,{','.join(map(str, circle))}\n")

# Main pipeline function
def main_pipeline(csv_path, output_csv_path):
    ...
    # Detect circles
    detected_circles, circles_img = detect_circles(paths_XYs)
    ...
    # Save results to CSV
    save_results_to_csv(regularized_corners, filtered_lines, detected_circles, output_csv_path)
    print(f"Results saved to '{output_csv_path}'.")

# Example usage
csv_path = 'problems/isolated.csv'
output_csv_path = 'detected_shapes.csv'
main_pipeline(csv_path, output_csv_path)
